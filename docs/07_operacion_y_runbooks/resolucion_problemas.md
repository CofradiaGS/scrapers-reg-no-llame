...
1. **Auto-Sanación Nativa:**
   En [adapters/scrapers/iris/iris_http_bot.py](file:///c:/Users/automatizacion.crm/Documents/GitHub/scrapers-reg-no-llame/adapters/scrapers/iris/iris_http_bot.py#L156-L161), el método `_open_query_screen` intercepta automáticamente el mensaje `SESSION_TIMED_OUT` o redirecciones a `login.xhtml` y reautentica la sesión de inmediato sin abortar el worker:
   ```python
...
2. **Auditoría de Credenciales:**
   Verificar que las credenciales en [.env](file:///c:/Users/automatizacion.crm/Documents/GitHub/scrapers-reg-no-llame/.env) no hayan expirado:
   ```powershell
