...
1. **Auto-Sanación Nativa:**
   En [adapters/scrapers/iris/iris_http_bot.py](file:///c:/Users/Usuario/Documents/GitHub/scraper%20iris%20reg%20no%20llame/adapters/scrapers/iris/iris_http_bot.py#L156-L161), el método `_open_query_screen` intercepta automáticamente el mensaje `SESSION_TIMED_OUT` o redirecciones a `login.xhtml` y reautentica la sesión de inmediato sin abortar el worker:
   ```python
...
2. **Auditoría de Credenciales:**
   Verificar que las credenciales en [.env](file:///c:/Users/Usuario/Documents/GitHub/scraper%20iris%20reg%20no%20llame/.env) no hayan expirado:
   ```powershell
