...
class MovistarAdapter(BaseScraperAdapter):
    ...
    def iniciar(self) -> None:
        ...
        if self.use_proxy_pool:
            ...
            if self.tor_controller.is_running():
                self.proxy = self._generar_proxy_aislado()
                logger.info(f"MovistarAdapter enrutando por Tor Stream Isolation (Slot {self.worker_slot or 1}).")
        ...
        if self.proxy:
            ...
            logger.info(f"MovistarAdapter configurado con proxy: {self.proxy[:30]}...")
    ...
    def consultar_linea(self, linea: Linea) -> ScrapeResult:
        ...
        try:
            ...
            logger.error(f"Error de red al consultar Movistar en Cobro Express para {linea.ani}: {e}")
            raise RuntimeError(f"Error de red en MovistarAdapter: {e}")
        except Exception as e:
