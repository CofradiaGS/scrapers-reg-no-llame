# -*- coding: utf-8 -*-
"""
Adaptador Secundario (Driven Adapter): Scraper Personal Argentina (Cobro Express)
Implementa IScraperEnginePort interactuando con la API REST de pagosce.cobroexpress.com.ar.
Consulta por Línea telefónica directa de 10 dígitos (Modalidad 2864, campo CBF) sin requerir DNI previo.
...
logger = logging.getLogger("PersonalAdapter")


class PersonalAdapter(BaseScraperAdapter):
    """Adaptador de producción para Telecom Personal Argentina a través de Cobro Express."""

    ID_EMPRESA = 20910           # Telecom Personal
    ID_EMPRESA_MODALIDAD = 2864  # Cobranza sin factura por número de línea (campo CBF)
    ...
    def iniciar(self) -> None:
        ...
        if self.use_proxy_pool:
            ...
            logger.info(f"PersonalAdapter enrutando mediante ProxyPoolManager ({self.proxy_pool.size()} proxies vivos).")
    ...
    def consultar_linea(self, linea: Linea) -> ScrapeResult:
        """
        Consulta la línea en la API de Cobro Express bajo la modalidad Telecom Personal.
        ...
        try:
            ...
            if resp.status_code == 400:
                ...
                return ScrapeResult(
                    ...
                    descripcion=f"Sin coincidencia en Personal: {mensaje_error}"[:195],
                    detalles={
                        "http_status": 400,
            ...
            nom_empresa = (primer_item.get("nombreEmpresa") or "TELECOM PERSONAL").strip()
            ...
            detalles_completos = {
                ...
                "comprobantes": items,
                "comprobantes_enriquecidos": items_enriquecidos,
                "ani_consultado": linea.ani,
                "dni_consultado": dni_titular,
            }

            desc = f"Personal - Cliente: {id_cliente} | Deuda: ${total_deuda:.2f} | Vto: {vencimiento}"[:195]
